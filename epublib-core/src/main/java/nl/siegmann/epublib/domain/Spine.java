 ...
 * 
 */
public class Spine implements Serializable, Parcelable
{
	 ...
	 * @param source
	 */
	public Spine(Parcel source)
	{
		if (source.readByte() == 1) tocResource = source.readParcelable(Resource.class.getClassLoader());
		if (source.readByte() == 1) source.readTypedList(spineReferences, SpineReference.CREATOR);
	}
	...
	/**
	 * As per the epub file format the spine officially maintains a reference to the Table of Contents. The epubwriter will look for it here first,
	 * followed by some clever tricks to find it elsewhere if not found. Put it here to be sure of the expected behaviours.
	 * 
	 * @param tocResource
	 */
	...
	{
		dest.writeByte((byte)(tocResource != null ? 1 : 0));
		if (tocResource != null) dest.writeParcelable(tocResource, 0);
		dest.writeByte((byte)(spineReferences != null ? 1 : 0));
		if (spineReferences != null) dest.writeTypedList(spineReferences);
	}

	public static final Parcelable.Creator<Spine> CREATOR = new Parcelable.Creator<Spine>()
	{
		@Override
		public Spine createFromParcel(Parcel source)
		{
			return new Spine(source);
		}
